...
# - 不要用 mcp__remote-supabase__execute_sql 執行 DDL（CREATE/ALTER/DROP）
...
- MCP 使用 `supabase_admin` 角色連線
- 建立的物件 owner 會是 `supabase_admin`
- CI/CD 使用 `postgres` 角色，無法修改這些物件
- 結果：migration 失敗 `must be owner of table xxx`

```sql
-- ❌ 禁止透過 MCP 執行
CREATE TABLE your_schema.new_table (...);
CREATE INDEX idx_xxx ON your_schema.table (...);
ALTER TABLE your_schema.table ADD COLUMN ...;

-- ✅ MCP 只能用於
SELECT * FROM ...;           -- 查詢資料
SELECT tableowner FROM pg_tables WHERE ...;  -- 檢查 owner
ALTER TABLE xxx OWNER TO postgres;  -- 修復錯誤的 owner
```

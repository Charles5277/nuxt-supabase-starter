...
本分支把**多件可平行的工作**各派一個 worker pane，再把**本 session 的整個位置**交給一個 successor——由它繼承那 N 筆 worker 的 coordinator 身分並回收本 pane，然後本 session 收工。
...
對每件工作跑一次，**不帶** `--coordinate`、**不帶** `--relay`：
...
`--relay` 轉移的是 `inFlightDispatchesOf()` 在**執行那一刻**掃到的 dispatch record（`parent_pane_id === 本 pane && parent_claude_session_id === 本 session && 無 correlated outcome`）。relay 之後才派的 worker 不會被任何人繼承，而本 pane 隨即被 successor 回收——那筆 worker 直接變成 orphan，它的 outcome 寫進 durable record 後**沒有任何東西會來收割**。

...
//   * bare:            node "${PROJECT_DIR}/.../hook.mjs"
...
//   ${PROJECT_DIR}  → the project root being scanned. This is exactly the
//                            runtime mapping (AI Agent sets it to the project
//                            dir at hook time), so we EXPAND it against `root`.
//                            Not doing so was the #402 bug: the literal
//                            `${PROJECT_DIR}/...` string never exists.
...
function resolveHookScriptPath(token, root) {
  ...
  const expanded = token.replace(/\$\{PROJECT_DIR\}/g, root)

...
## 選項 B 全流程（Phase B-0a Fable draft / B-0b Pi review / B-0c 主線 final check）
   ...
   3. **Write Pi review prompt 到 `/tmp/pi-spectra-propose-<change-name>-review-prompt.md`**：
      ...
      - .cursor/rules/ux-completeness.mdc（Affected Entity Matrix / User Journeys / Implementation Risk Plan / Fixtures-Seed Plan / Design Review 7 步）
      - .cursor/rules/manual-review.mdc（## 人工檢查 Item Kind Marker、step actionability、[verify:auto] 禁用）
      - .cursor/rules/agent-routing.mdc（Phase Purity：UI view phase vs 非 view phase 是否切開）

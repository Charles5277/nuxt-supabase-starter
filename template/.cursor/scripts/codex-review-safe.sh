...
# (plugins/hub-core/scripts/) and a consumer's projected copy (.cursor/scripts/)
...
Paths under `.cursor/rules/`, `.cursor/skills/`, `.cursor/agents/` and
`.cursor/commands/` are PROJECTIONS of the clade central repo. Their source of
...
if ! cmp -s "$WORK_DIR/worktree-before.txt" "$WORK_DIR/worktree-after.txt"; then
  ...
  if [ "$SCOPE_RC" -eq 0 ]; then
    ...
    echo "[codex-review-safe]   cd /tmp/\$(basename \"\$REPO_ROOT\")-review && bash \"\$REPO_ROOT/.cursor/scripts/codex-review-safe.sh\" <effort>" >&2

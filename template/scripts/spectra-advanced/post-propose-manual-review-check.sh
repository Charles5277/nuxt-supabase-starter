#!/usr/bin/env bash
# 🔒 LOCKED — managed by clade · Source: vendor/scripts/spectra-advanced/post-propose-manual-review-check.sh · 改這裡無效，下次 propagate 會覆寫；請改 $CLADE_HOME/vendor/scripts/spectra-advanced/post-propose-manual-review-check.sh
...
# has no tunnel configured (e.g. <consumer-d>) — return exit 1 → main loop
...
run_tunnel_check() {
  ...
  local host=""
  local source_env=""
  # Scan common env file names + any `.env.<app>` for multi-app consumers (<consumer-a>).
...
# 92-line ## 人工檢查 block (<consumer-a> line-messaging-interaction: 41 s idle, past
...
for i in $(seq 0 $((PATTERN_COUNT - 1))); do
  ...
  for pos in $matched_pos; do
    ...
    # consumers like <consumer-d>); enrich when tunnel exists.

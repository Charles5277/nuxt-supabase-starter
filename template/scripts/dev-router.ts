...
//
// 為什麼存在：tunnel（<consumer-a>-<client-a>-dev.<maintainer-domain>）固定指向一個公開 port，
...
//
// control UI：http://127.0.0.1:<controlPort>（controlPort = publicPort + 300；<consumer-a> <client-a> 3040→3340）
...
interface RouterState {
  ...
  backends: Record<string, BackendState | undefined>
}
...
async function daemonFetch(path, init?: RequestInit) {
  try {
...
async function runDaemon() {
  ...
  const children = new Map<string, ChildProcess>() // slug → child
  ...
  // ── 啟動序列 ──
  await new Promise<void>((res) => proxy.listen(publicPort, '127.0.0.1', res))
  await new Promise<void>((res) => control.listen(controlPort, controlHost, res))

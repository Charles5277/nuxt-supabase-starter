...
**Reference impl = TDMS**（`app/app.vue` + `app/plugins/page-loading.client.ts` + `app/composables/usePageLoading.ts` + `app/stores/ui.ts`）。本 rule 從 TDMS 萃取**架構**，但修掉其 warts（自刻 bar 跟 NuxtLoadingIndicator 重複、skeleton 用硬編碼 `bg-gray-*` 違反 color-mode rule）。
...
**Canonical pattern（對齊 TDMS reference）**：
...
- **MUST NOT** 自刻 top progress bar 跟 `<NuxtLoadingIndicator>` 功能重複（TDMS 的 `GlobalLoadingBar` 是反例；NuxtLoadingIndicator 已涵蓋）。
...
mutation `status === 'pending'` 當 loading 是**真 functional bug**（永久 spinner），比 perceived-perf 議題嚴重，且反覆再犯（TDMS 3 處、perno 30+ 處）。故 Tier 2.5 有**四層** enforcement，各管不同 scope：
...
- 導航 loading 是每個 Nuxt consumer 都會遇到的 cross-cutting concern，散規範必漂移：盤點顯示 TDMS 有完整 pattern、perno/starter 有分歧半套、yuntech / agentic-rag / rental-scout / co-purchase 全缺。

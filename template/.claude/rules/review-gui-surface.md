...
（`grok-xai` / `grok-cursor` / `astra` / `gemini` / `luna` 一律不准）、**NEVER** 走
...
佇列的實作契約（seen 集合、訂閱移除、推播 REQUIRED 三欄）住在 clade 端 `.claude/rules/local/decision-queue-contract.md`，不散播——consumer 端 agent 只需知道兩端同源，**NEVER** 自己另掃一份。

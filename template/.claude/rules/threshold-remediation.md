...
4. **複量結果仍 > 門檻的 90%（含仍超標）→ MUST 派 Fable 顧問檢討成長結構**，NEVER 自行再修一次邊。
   顧問 brief 帶：門檻與歷次實測值、本次處置手法與降幅、前幾次同型處置的結果。
   `Agent({ subagent_type: 'Plan', model: 'fable', run_in_background: false })`，read-only（brief 明寫
   「只回建議與理由，NEVER 改任何檔」）。反覆長回門檻的檔案，成因在**寫入契約**而不在存量，
   而那一層不是再壓縮一次能碰到的。

...
- 已驗證移除安全：2026-05-23 對 4 個 Supabase consumer ripgrep `hub*()` API 呼叫 = 0；2026-05-26 對 <consumer-i>（void.cloud migrate 後）同樣驗證 `hub*()` 呼叫 = 0
...
走 [void.cloud](https://void.cloud) VoidZero 部署平台的 consumer（建在 Cloudflare Workers 上，但 deploy pipeline 由 void 接管）。Fleet 採用：<consumer-l>、<consumer-i>、quotation-generator（migration 中）。
...
→ Fleet 為什麼沒事到 2026-05：所有 wrangler-action consumer 讀 `.output/server/wrangler.json`，flag 自動對齊；只有 void.cloud track 暴露 void.json 與 wrangler.json 的 flag 落差。<consumer-l>（2026-05-25）+ <consumer-i>（2026-05-27）兩例撞 10021 後 promote 成本 §。
...
**Fleet 採用**：<consumer-k>（<consumer-i> 2026-05-26 前採此 pattern，遷 void.cloud 後改走 § 4.5）
...
**Fleet 採用**：<consumer-l>、<consumer-i>、quotation-generator（migration 中）
...
| <consumer-k> | Cloudflare D1 | A (wrangler-action) | ✅ | n/a |
...
| **<consumer-i>** | Cloudflare D1（void 自管）| **B (void.cloud)** | ❌ | legacy 0.8：配置 3（v1 + no_v2）+ patch-void-deploy；待升 current void 後退役 patch |
...
YuDefine fleet 多個 consumer（<consumer-k> / <consumer-i> / <consumer-l>）的 CI / deploy workflow 跑在自架 runner `runs-on: [self-hosted, gh-runner-lxc]`（YuDefine LXC，省 GitHub-hosted minutes）。Self-hosted runner 跟 GitHub-hosted runner 有兩個關鍵差異，沿用 GitHub-hosted 的 workflow 寫法會踩坑：
...
| --- | --- | --- | --- | --- |
| <consumer-i> | self-hosted, gh-runner-lxc | ✅ | ✅ | legacy VOID_TOKEN；待遷 GitHub OIDC |

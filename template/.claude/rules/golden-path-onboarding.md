...
3. **`self-hosted-runner` 是 informational**：wrangler / Cloudflare 型 consumer 用 `ubuntu-latest` 合理，不因此判 drift。只有走 Docker self-hosted deploy 型（perno / TDMS / yuntech-usr-sroi / cnc-link-*）才 MUST 用 `[self-hosted, ...]` runner。

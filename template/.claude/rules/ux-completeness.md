...
| ----------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------- |
| Before `spectra-propose` (handoff)  | **AskUserQuestion**：A. Codex GPT-5.6-sol xhigh / B. Claude Code 繼續做；A 路徑由主線 Claude 自己以背景 dispatcher 派 Codex 模型（Pi transport）（**禁止**叫使用者切 CLI） | discuss 結束、進入 propose 前必詢問；由 `spectra-propose` Step 0 統一分流，詳見 `agent-routing.md` |

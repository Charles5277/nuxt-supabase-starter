...
**為什麼拆**（2026-08-02，實測）：`code-style.md` 原本 27.3 KB、`paths:` 吃全 repo 的
...
`.github/workflows/**` 時違反。**規約會在違規動作發生的那一刻在場**，這是拆得掉的原因——
...
`pnpm-lock.yaml` 重生時 oxlint patch 升版可能讓既有 warning 升 error（perno 2026-05-14 觀察到 `vp lint` 對 `scripts/audit-ux-drift.ts` 從「2 warnings + 0 errors」變成「0 warnings + 1 error」）。CI lint baseline 需週期性 audit。
<!-- starter:strip-end -->
...
<!-- starter:strip-begin -->
**真實事故參考**：perno 2026-05-14 觀察 `vp lint scripts/audit-ux-drift.ts`（檔案內容無 git diff）：
...
- `scripts/audit-tooling-drift.ts` `eslintDeps` signal：掃每個 consumer `package.json` deps/devDeps 是否含本節禁用的 eslint / prettier 套件（`eslint` / `@nuxt/eslint*` / `@typescript-eslint/*` / `eslint-config-*` / `eslint-plugin-*` / `prettier` / `prettier-plugin-*`）→ 列為 `forbidden-deps-present`。warn-only，源於 rental-scout + co-purchase 採 Vite+ 後移除 `@nuxt/eslint`

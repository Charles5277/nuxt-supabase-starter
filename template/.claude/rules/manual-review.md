...
**既有 `[x]` ≠ 已驗收**：archive / 收尾前遇到 `[review:ui]` 已是 `[x]` 但無對應 agent 自拍 screenshot evidence（`screenshots/local/<change>/#<id>-*.png`）時，一律視為 **false-green**，主線 **MUST** 無視 checkbox 自起 dev server + 依環境自拍自驗（Cursor → `cursor-ide-browser`；非 Cursor → `agent-browser`；跨 session 也自足），**NEVER** 假設 user 有截圖或信任前 session 代勾 — 自拍動作本身會撞出被 checkbox 掩蓋的 bug。詳見 [[agent-self-verification.screenshot-evidence]] MUST 8 + [[pitfall-review-ui-checkbox-without-agent-evidence-masks-bug]]。

...
>
> **Workspace capability 與路徑可見性是兩個獨立 predicate。** **每一個**會修改 working tree、lockfile、Git index 或建立 commit 的 Pi caller／brief都屬 `mutation`：concrete table row 由 `pi-routing-policy.ts` 分類，manual caller **MUST** 帶 `--workspace-access mutation`；quota retry **MUST** 沿用 dispatcher 的 `next_step`／`--retry-of`，由 ledger 繼承同一 capability。只讀 inspection／review 才是 `readonly`；manual caller 要進 Cursor pool 時顯式帶 `--workspace-access readonly`。
>
> **違反字面就是違反精神：任何 workspace mutation dispatch，NEVER 選 `grok-cursor`、`luna-cursor` 或 `sol-cursor`，包含每一個 fallback candidate。** `pi-dispatch.ts` 在 Cursor admission fail closed，mutation chain 計算時直接跳過所有 `*-cursor`；capability 無法判定時也不猜成 readonly。即使 brief 的所有檔案都在 cwd 內、linked common gitdir 也已可見，答案仍相同：`buildCursorSandboxArgv()` 對 root、cwd 與額外 bind 全用 `--ro-bind`。**NEVER** 為了 mutation 放寬這道 security boundary。成因與實證邊界見 `docs/pitfalls/2026-08-30-cursor-readonly-sandbox-cannot-carry-worktree-mutation.md`。
>
> | 開脫句 | 現實 |
> | --- | --- |
...
>
> **Red Flags**：發現自己把錯誤讀成「mount 缺一條」、準備加 writable bind、或準備先派 Cursor 等 EROFS 再 fallback → 停；回到 `workspace_access` 分類，讓 dispatcher 在派工前排除 carrier。
>
...
>
> **每一次** pi dispatch **MUST 帶 `--route` 與 `--tier-basis`**（缺就 exit 1）：前者記走哪條政策（本表某列 → `routing-table`；§ Claude 委派的 model 檔位 → `claude-delegate-sub`；配額降級鏈 → `fallback-chain`；皆非才**顯式** `manual`），後者記該政策對 model 的**結論**（六值見 reference）；dispatcher 交叉檢查兩者與 `--model`、矛盾即 exit 1。**NEVER** 不確定就填 `manual` ／ `table-row`——與「判定沒發生」不可區分。重試帶 `--retry-of <label>`，**NEVER** 用 `<label>2`。
>
> **`--tier-basis table-row` 時 MUST 再帶 `--table-row <列名>`**（缺就 exit 1）：列名是下表每列開頭 〔`如此標示`〕 的 slug，dispatcher 拿該列的 model 交叉檢查。**NEVER** 因為「反正 table-row 也是查表」就省略它（它曾是唯一對 model 零約束的 basis，實錄見 rationale）。**NEVER** 在派工當下偏離列上的 model —— 認為某列該換檔位就先改本表再派。
>
...
| 〔`copywriting-draft`〕 **行銷／產品對外文案的草稿與變體生成**（標題變體、商品描述、社群貼文、EDM、SEO meta、landing page 段落初稿）。**只收候選素材**：主線會整份重寫才走本列 | **Pi `--model gemini --effort low` via 泛用 dispatcher**（`gemini-3.7-flash`）。**exit 2／3／4 一律直接回主線 Opus 5 自己寫**，本列 NEVER 進全域配額降級鏈 | **主線 Opus 5 MUST 收斂重寫每一條採用的文案，NEVER 原樣貼進交付物**——Pi 回的是素材不是成稿。繁中語感、台灣慣用語與品牌 tone 的最終判定留主線。**本列只涵蓋行銷／產品對外文案**：規約措辭、commit message、技術文件、PR 描述、對外報告一律留主線（per § 派不派 不外派清單），**NEVER** 從本列外推到那幾類。降檔救不回語感，所以不進配額鏈——成本論證見 rationale。 |
| 〔`notion-ops`〕 **Notion 讀寫**（`ntn api` 查詢／建頁／改狀態／comment；`/notion-ticket` outbound、`/notion-board` inbound、spectra-notion-coupling 狀態推進） | **Pi `--model gemini --effort high` via 泛用 dispatcher**。**exit 2／3／4 → `--model luna` 同 effort**（`--route fallback-chain --tier-basis quota-fallback --retry-of <label>`，**不帶** `--table-row`）。luna 再失敗 → 主線 Opus。**本列 NEVER `luna-cursor`**（auth 在 `$HOME`）。luna 的 exit 4 payload 會寫 `next_tier: luna-cursor`——**本列忽略該格，直接回主線**（payload SoT 的具名例外）。**NEVER** 續走 grok-xai／Haiku | ntn 是 CLI，gemini／luna 做得到；cursor 池 `$HOME` 是空 tmpfs，拿不到 `~/.config/notion/auth.json`。仍只靠 claude.ai MCP 才做得到的殘集不走本列、留主線。客戶看得到的 ticket 正文是**定稿措辭**：Pi 起草後主線 MUST 收斂重寫才寫入。Board scan／status patch／query 是事實表，不必重寫。**NEVER** 主線第一手自己跑 ntn／MCP。 |
...
| 〔`debug-evidence`〕 **Debug evidence 段**（log 完整 capture / repro script 撰寫執行 / 既定 hypothesis 的驗證迴圈） | **Pi `--model sol --effort high` via 泛用 dispatcher** | evidence / repro / verify 是機械段，root cause 推斷與修法設計留主線。repro 必在 throwaway worktree（template 內建 guard）。 |
| 〔`commit-0c-fix-verify`〕 **commit 0-C fix-verify loop**（pnpm check / test 修到全綠） | **Pi `--model grok-xai --effort high` via 泛用 dispatcher**（`xai/grok-4.6`）。同一 dispatch 最多 **2** 輪 check→fix（`--var max_iterations=2`）。本列是 workspace mutation；exit 4 **MUST** 照 capability-aware payload 跳過 `grok-cursor` 並進〔`commit-0c-fix-verify-escalate`〕sol升級列 | 機械修 lint / type / test。grok 無 class-conditional 取證，用次數上限接住；主線重跑 check **不信自報**。見 commit SKILL Step 0-C。 |
...
- **輸出本身就是 gate 的工作，鏈的終點 NEVER 是主線自審**。判準見
  ...
  產出 changeset 的那條主線回頭審自己，gate 形式上補了位、實質是空的。一般 gate row 在 Sol
  兩池耗盡後由 dispatcher 指向 **Fable subagent**（`--model fable`，effort `max`）；
  `PI_GATE_UNMET_ON_SOL_EXHAUSTION_ROWS` 是更嚴格的子集：`spectra-artifact-review` 的 reviewed
  ...
  都不得補位。其餘非 gate row 才維持 Opus 主線。`-cursor` 那一跳照走，它換的是配額池不是家族。

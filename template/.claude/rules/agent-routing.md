...
2. **否則** → Herdr create-only：`node vendor/scripts/herdr-session-handoff.ts --cwd <abs> --label <label> --prompt-file <brief> --model <slug> --effort <level> --launcher cc` 或 `--launcher ccw`，並加 `--coordinate`。`--model` 必填：要 child 跑特定 model 就給 slug，brief 正文寫「用 Opus 5」child 做不到。**NEVER** `--relay`。clade / YuDefine 工作預設 `ccw`，其餘 `cc`。Cursor 沒有 `CLADE_CLAUDE_LAUNCHER`，缺 `--launcher` 會 `unsupported_launcher`。**NEVER `--launcher pi`**：create 一律拒（Pi 新工作走 `pi-dispatch.ts`），Cursor 池連 relay 都拒。
...
每次 dispatch/resume/retry/bridge **MUST** 按 Routing Table 或使用者 override 明傳 model/effort；**NEVER** inherit、取環境/主線預設或偷換模型。Claude 用 Claude Code，GPT/Gemini/Grok 工作用 Pi。requested 與 observed 分記，無 runtime 證據就是 unverified。

使用者指定 Pi Luna/medium 修復時：
...
Herdr 不增加派工或跨界授權；能原地完成就原地做。**MUST** 在換 session 前讀 [[session-tasks.operations]] § Herdr session transport，保留 scope/approval/verification 邊界；失敗不退回人工操作。Pane 缺席不等於無工作，查 `vendor/scripts/herdr-patrol.ts`。
...
| 〔`screenshot-review-verify`〕 | **本列 NEVER 派 Pi 任一 model**——`grok-xai` / `grok-cursor` / `astra` / `gemini` / `luna` 一律不准，`pi-dispatch-screenshot-verify.ts` 已 fail-closed 拒跑。**Cursor runtime 例外**：NEVER 用 Cursor Task 的 `model=claude-*` 假裝本列；改走 § Cursor runtime 主線 residency 的 Herdr create-only `cc`／`ccw`。**NEVER 恢復「subagent 再轉派」**的形狀。 |
| 〔`screenshot-match-analysis`〕 | 收集與判定是兩個角色，**NEVER** 併成同一次 dispatch。 |
| 〔`mechanical-fanout`〕 | **NEVER** 以「我自己順手跑掉比較快」略過本列（成因見 rationale）。 |
| 〔`copywriting-draft`〕 | 本列 **NEVER** 進全域配額降級鏈（exit 2／3／4 一律直接回主線 Opus 5 自己寫）。**主線 Opus 5 MUST 收斂重寫每一條採用的文案，NEVER 原樣貼進交付物**——Pi 回的是素材不是成稿。本列只涵蓋行銷／產品對外文案，**NEVER** 從本列外推到規約措辭／commit message／技術文件／PR 描述／對外報告。 |
...
**撞到這道 gate、要決定怎麼結案之前，MUST 先讀 [[agent-routing.pi-watch-protocol]] § Routing threshold 與 Claude Agent dispatch gate**——正常結案的逐字 `pi-dispatch.ts` 指令與 effort 對照、四個具名 waiver、Gemini／Astra exit 2 的升級鏈與 exit 4 的配額鏈，都在那裡，**此處不複述**。

**本節路徑的 luna 准入（與 § Routing Table 五條連言無關）**：原判 `sonnet` 的委派 MUST 同時滿足兩項——(1) § MUST 指定 `model: 'sonnet'` 的四條 predicate 全中（複核，不是加碼）；(2) 未命中 § NEVER 降檔的形狀任一條。兩項都過 → 派 `--model gemini --effort high`，**NEVER** 因「gemini 較便宜」自行改回 `--model astra` 或退回 Claude `sonnet`。任一項不過 → 照原判派 Claude `sonnet`（顯式帶 `model`），**NEVER** `--model terra`。

**gemini 回 exit 2（業務 fail）→ 升 `--model astra` 重派一次：原 low 維持 low，其餘 effort 改 medium**；再 fail 才回 Claude `sonnet` subagent。exit 3／4 照 § 配額耗盡時的 fallback 紀律 與 watch-protocol 的 exit code 分流走，**NEVER** 記入品質判斷。
...
Routing Table 決定誰**寫** code，本節決定誰**持有長 session**（成本結構見 rationale）。依 change 特性二選一：

### Codex-primary（Codex 扛整條 session）

**進入條件**（A 或 B 命中即走）：

- **A. Spectra apply 已可執行**：tasks.md 已定稿，工作性質已從「釐清需求」進入「執行已知計畫」。是否含 UI view phase **不參與 residency 判定**；它只決定 bounded phase executor。
- **B. 機械式 sweep**：lint fix / dep upgrade / rename / cross-file refactor / test 修復 / codemod，即使無正式 tasks.md。

**長 session carrier 與 phase executor 是兩題。** Codex-primary 持有 change-level orchestration、tasks 狀態與跨 phase 歷史；UI／Design Review／其他需 Claude harness 的 bounded phase 仍依 § Spectra Apply Phase Dispatch 交給 `cc`／`ccw`，完成後把結構化結果送回 carrier。**NEVER** 用「含 UI」直接推出「整條長 session 留在 Claude Code」——那會把 Claude Code 的 resident floor 與整條 apply 歷史綁在一起，正是 ccx 退役暴露的失敗形狀。

**做法是 change 粒度不是把每個非-view phase各開一個新 carrier**：一次由單一 Codex-primary session 持有整條 change；各 phase 的實作者照 capability routing 決定。四步做法（dispatch / notification-only watch / change 粒度 cross-check / bounded Claude phase 回收）全文在 reference § Orchestration Residency — 機械 Enforcement，**開工 dispatch 之前 MUST 先讀那一節**。
...
進入 apply 後，個別 phase executor 仍走 § Spectra Propose / Apply Dispatch；那張表決定誰實作，不反向改寫 residency。

### 機械 Enforcement

**每一條** change 開工都 **MUST** 跑 `residency-classify.ts classify` 拿機械 verdict 並立刻 `record`，不是只有看起來像純後端的那條——「主線自行判斷 residency」已實證不可靠。缺 record 會被 archive-gate Check 8 擋（exit 2）。命令與繞過 marker 見 reference 同名 §。
...
**本節射程只涵蓋 Claude Code 內建的 `WebSearch` / `WebFetch`。** `cursor-ide-browser`（`browser_navigate` 等）與 `cursor-app-control` **不是**這兩支 tool，**NEVER** 套本節去擋 IDE browser。Cursor 環境的開頁契約見 § Cursor 環境的 browser 載體。
...
- **輸出本身就是 gate 的工作，鏈的終點 NEVER 是主線自審**。判準見
  ...
  都不得補位。其餘非 gate row 才維持 Opus 主線。Astra 目前沒有已驗證的 Cursor model，直接採用上述終點。
...
4. **Model 與 effort 顯式指定**：**每一個** dispatch 都 MUST 把 model 與 effort 當成兩個獨立決策，不靠靜默繼承——省略 = 繼承主線（通常最貴檔 × 最深推理），機械掃描型 subagent 拿主線的 xhigh 跑就是效能過剩。選檔預設，依序判：
   ...
   - **effort 選檔**：Astra 依 named row 使用 `low` 或 `medium`；原 Sol 的 `low` 對應 Astra `low`，其餘 effort 對應 Astra `medium`。其他模型：機械掃描／純轉錄 → `low`；一般執行 → `medium`；判讀型／高錯誤成本 → `high` 以上。**帶得了 effort 參數的入口**（pi `--effort` / `-c model_reasoning_effort`、Workflow `agent()` 的 `effort`、具名 agent type 的 frontmatter）**MUST** 顯式帶；Agent tool 本身沒有 effort 參數、只能繼承主線——這是機械型工作優先走 Pi 而非 Agent tool 的另一個理由
...
| **NEVER** 在 exploration / research 型 session 自己逐檔 Read + scan 多個 source（openspec / HANDOFF / git log / docs）超過 3 個 source file | 先派 Pi `astra low` pre-scan 拿 structured summary，再由主線消費 summary 做判斷。例外：user 明確問特定檔案 / 需要 claude.ai-connected MCP |
...
commit 0-A 的四條 dispatch 禁令（simplify 與 pi 不並行、不啟用已棄用的 `code-review` Opus subagent、不跑第 3 輪 pi、不用 `Agent` 包一層跑 simplify）全文在 commit skill 的 `gates.md` § 0-A —— **跑 0-A 之前 MUST 先讀那一節**。

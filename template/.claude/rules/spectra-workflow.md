...
| --- | --- | --- | --- |
| **OPSX intent** | manual-review item data-readiness：sample key inline + **該 key 是否真的會在 target UI render**（reverse page-grep） | runtime 正確性、視覺 | **Layer A** `VERIFY_UI_SAMPLE_KEY_DISPLAY_CHECK` + `page-display-check.mjs` |
...
| **OPSX execution — pre-handoff** | 5 維度跨檢（task↔render / evidence↔dom fab / list↔fallback / api boundary / error tail），主線 + codex 跨模型 | — | **Layer E.1**（主線 self-analysis）+ **E.2**（codex cross-check） |
